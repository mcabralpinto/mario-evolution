...
Evolved champions are compared against a random-search baseline to isolate the effect of evolution from grammar expressiveness. We run five independent experiments per task (different random seeds). In each run, the evolved champion is selected from checkpoints by win rate (Runner), average kills (Hunter), or average coins (Thief); the baseline champion is selected analogously from checkpoints of 100 random individuals by best held metric. Both are then evaluated on $5{,}000$ held-out level seeds across three difficulty levels. Across the five runs, we report mean and standard deviation for completion rate, average distance in unbeaten levels, enemies killed, and coins collected.
...
Table~\ref{tab:comparison} compares evolved champions against the random search baseline across all three task variants, evaluated on $5{,}000$ held-out seeds $\times$ 3 difficulties. Table~\ref{tab:winrate_diff} displays win rate by difficulty, showing non-conditional and conditional rates.
...
\midrule
Levels Cleared (\%) & $0.3 \pm 0.5$ & \textbf{39.3 $\pm$ 0.2} & $32.2 \pm 0.6$ & $30.5 \pm 0.5$ \\
Max Distance (px)   & $458.9 \pm 53.4$ & \textbf{1005.3 $\pm$ 2.5} & $942.3 \pm 10.6$ & $928.9 \pm 17.8$ \\
Enemies Killed      & $0.07 \pm 0.16$ & $0.63 \pm 0.06$  & \textbf{0.88 $\pm$ 0.04}  & $0.83 \pm 0.03$  \\
...
All three evolved agents outperform the random baseline in all metrics, given that it struggles to complete any level and usually winds up dying early or getting stuck on a wall due to not learning to jump. It also displays high standard deviations across all metrics, as it does not obtain a solidified strategy across individuals, unlike the other tasked agents, which consistently leans toward the variation of the ``run-jump'' strategy that is better suited for its task. The fact that Runner has a considerably higher win rate than Hunter and Thief aligns with the expected behavior: while all incentivize forward progress, Runner directly rewards beating levels. However, Hunter and Thief aren't too behind: their secondary goals appear to serve as an effective proxy for winning levels (the further they reach in the level, the better their chances of more kills/coins). 

For kills and coins, Hunter and Thief outperform Runner in both metrics. We theorize that, due to individuals of the former two tasks tendentially being slower (using the \textsc{Speed} control less), they cover more space and have more opportunities to interact with enemies and coins (thus also dying more frequently to enemy non-kills and pits). Between the two, Hunter is the most effective at utilizing this strategy, even collecting more coins than Thief. This may also be explained by the fact that there is no mapping for coins in the perception grid, essentially meaning it is much harder to optimize for this goal. 

Table~\ref{tab:winrate_diff} shows a consistent difficulty spike across all tasks: win rates fall from around $65\%$ to $25\%$ and then to $12\%$. In most cases, the conditional rate exceeds the non-conditional rate, meaning that beating the previous level is predictive of beating the current one. However, for Runner, the conditional rate at $d=2$ is $14.6\%$ against a non-conditional $16.3\%$, a result that is statistically significant (p < 0.05). This may indicate that the additional challenges introduced by $d=2$ compared to $d=1$ may require a slightly different skill set to beat, and that beating $d=1$ is not a reliable predictor of beating $d=2$ when using this strategy.

In Table~\ref{tab:pairwise} we make several pairwise comparisons between task types to isolate task-specific gains. The first two comparisons confirm extremely large advantages. The last two show that while Hunter's advantage over Thief on killed enemies is statistically significant, the advantage on coins is not.

\vspace{-0.4cm}
\begin{table}[htbp]
\centering
\caption{Pairwise comparisons of key metrics across agents. Covers mean difference \(\Delta\), 95\% confidence interval for \(\Delta\), Cohen's \(d\), and result of a two-tailed Welch \(t\)-test.}
\vspace{-0.2cm}
\label{tab:pairwise}
\resizebox{\textwidth}{!}{%
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{../../data/search_space_analysis_full_reeval/heatmap_height_vs_mean_and_max_fitness.png}
  \caption{Mean-fitness heatmap across generations and tree heights. Each cell aggregates individuals of a given height at a given generation. Warm colors indicate higher fitness.}
  \label{fig:height_mean_fitness_heatmap}
\end{figure}

Figure~\ref{fig:height_mean_fitness_heatmap} shows that early generations are concentrated at lower heights and mostly poor mean fitness, while from roughly generation 100 onward the population spreads toward medium heights (around 9--13), where better averages appear more consistently. Very tall trees remain sparse and unstable, with occasional good values but no sustained advantage, which is consistent with the idea that moderate structural complexity is sufficient for useful policies while larger trees mainly add variance.
Hence the importance of bloat control through selection rather than a hard height limit: it allows the population to explore more complex structures while keeping them in check, rather than forbidding them.
...
GP over a typed grammar produced controllers that clear levels at 39.3\% across $5{,}000$ held-out seeds, against 0.3\% for random search. Hunter led on kills ($0.88$ vs.\ $0.63$ for Runner) and incidentally on coins, though the coin advantage over Thief is not statistically reliable. All three converge on ``run-jump'' variants, with the secondary objective shaping which variant is preferred. The biggest gains came from a small number of decisions: splitting the combined secondary objective into separate tasks, iterating the kill detector to eliminate false positives from the radius heuristic, and rotating the seed pool every 10 generations to prevent level overfitting without disrupting selection.

The grammar's absence of arithmetic or loops is the main structural limitation: the airtime penalty experiment showed agents struggle to evolve a conditional jump policy without richer perceptual primitives. 

\begingroup
\renewcommand{\thefootnote}{}
\footnotetext{\textbf{Note on AI Usage}: Claude assisted in LaTeX formatting and analysis verification.}
