...
The evolutionary process and level generator are both stochastic, so a single run per task is not sufficient to draw reliable conclusions~\cite{doe_lecture}. Across the five independent runs per variant, we report mean and standard deviation for four metrics:
\begin{itemize}
...
To quantify the advantage over the random baseline, we conducted paired-sample $t$-tests on completion rates, enemies killed, and coins collected for each task variant. The results indicated that \TODO{insert whether differences were statistically significant and in which metrics}.
...
Table~\ref{tab:comparison} compares evolved champions against the random search baseline across all three task variants, evaluated on $5{,}000$ held-out seeds $\times$ 3 difficulties ($15{,}000$ episodes per agent).
