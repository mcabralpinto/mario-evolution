...
1. **Launch the Evaluation Server**: First, run `docker compose up` in your terminal to launch the evaluation server.
2. **Wait**: Please wait about a minute for the server to fully start.
3. **Verify Server**: Go to [http://localhost:3000](http://localhost:3000) in your browser to see if the server is running correctly.
4. **Test the Setup**: Run the random search files to see if everything is running properly.

## Instructions for Students

- **Tasks Folder**: You should edit the `tasks` folder. This folder contains the **reward function**, which will act as the fitness for your algorithms.
- **Agents Module**: You can edit and create your agents in the `agents` module.
