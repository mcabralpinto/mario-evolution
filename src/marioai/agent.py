...
class Agent(object):
    ...
    def sense(self, obs):
        ...
        # obs is now an Observation namedtuple
        if obs.status != 0:
            # Episode over (Fitness packet)
            self.episode_over = True
        else:
            self.can_jump = obs.may_jump
            self.on_ground = obs.on_ground
            self.mario_floats = obs.mario_pos
            self.enemies_floats = obs.enemies
            self.level_scene = obs.level_scene
